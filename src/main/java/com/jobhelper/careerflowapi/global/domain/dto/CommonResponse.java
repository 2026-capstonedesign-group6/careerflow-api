...
public record CommonResponse<T>(
    ...
    public static <T> CommonResponse<T> onCreated(T data) {
        return new CommonResponse<>(Result.SUCCESS, "201", "리소스가 성공적으로 생성되었습니다.", data, LocalDateTime.now());
    }

    public static CommonResponse<Void> onNoContent() {
        return new CommonResponse<>(Result.SUCCESS, "204", "요청이 성공적으로 처리되었습니다.", null, LocalDateTime.now());
    }
    ...
    public static CommonResponse<Void> onFailure(ErrorCode errorCode, String detailMessage) {
        return new CommonResponse<>(Result.FAIL, errorCode.getCode(), errorCode.getMessage() + " " + detailMessage, null, LocalDateTime.now());
    }
